...
bool verifyResults(const std::vector<float> &expected, const std::vector<float> &actual,
    ...
    for (size_t i = 0; i < expected.size(); ++i) {
        ...
        if (rel_error > tolerance) {
            std::cout << "Mismatch at index " << i << ": expected " << expected[i] << ", got "
                      << actual[i] << ", relative error: " << rel_error << std::endl;
            return false;
        }
    }
...
int main() {
    ...
    float cublas_fp32_time = benchmark_kernel(
        [&]() {
            CHECK_CUBLAS(cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, d_B,
                                     N, d_A, K, &beta, d_C, N));
        },
    ...
    float cublasLt_fp32_time = benchmark_kernel(
        [&]() {
            CHECK_CUBLAS(cublasLtMatmul(cublasLt_handle, operationDesc, &alpha, d_B, Bdesc, d_A,
                                        Adesc, &beta, d_C, Cdesc, d_C, Cdesc, nullptr, nullptr, 0,
                                        0));
        },
        warmup_runs, benchmark_runs);
    ...
    float cublas_fp16_time = benchmark_kernel(
        [&]() {
            CHECK_CUBLAS(cublasHgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha_half,
                                     d_B_half, N, d_A_half, K, &beta_half, d_C_half, N));
        },
        warmup_runs, benchmark_runs);
    ...
    float cublasLt_fp16_time = benchmark_kernel(
        [&]() {
            CHECK_CUBLAS(cublasLtMatmul(cublasLt_handle, operationDesc_half, &alpha_half, d_B_half,
                                        Bdesc_half, d_A_half, Adesc_half, &beta_half, d_C_half,
                                        Cdesc_half, d_C_half, Cdesc_half, nullptr, nullptr, 0, 0));
        },
        warmup_runs, benchmark_runs);
    ...
    float naive_cuda_time = benchmark_kernel(
        [&]() { naiveMatrixMultiply<<<gridDim, blockDim>>>(d_A, d_B, d_C, M, N, K); }, warmup_runs,
        benchmark_runs);
    ...
    std::cout << "cuBLAS FP32 results " << (cublas_fp32_correct ? "match" : "do not match")
              << " the naive kernel results within tolerance of 1e-2." << std::endl;
    std::cout << "cuBLASLt FP32 results " << (cublasLt_fp32_correct ? "match" : "do not match")
              << " the naive kernel results within tolerance of 1e-2." << std::endl;
    std::cout << "cuBLAS FP16 results " << (cublas_fp16_correct ? "match" : "do not match")
              << " the naive kernel results within tolerance of 5e-1." << std::endl;
    std::cout << "cuBLASLt FP16 results " << (cublasLt_fp16_correct ? "match" : "do not match")
              << " the naive kernel results within tolerance of 5e-1." << std::endl;
